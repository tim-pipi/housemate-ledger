...
export function ExpenseForm({
  ...
  initial?: {
    ...
    values: Record<number, number>; // per-member input value in form units
    participants: number[];
  };
}) {
  const router = useRouter();
  const [state, formAction] = useFormState<FormState, FormData>(saveExpense, undefined);
      ...
      <div className="grid grid-cols-2 gap-3">
        <label className="col-span-2 flex flex-col gap-1 text-sm font-medium">
          ...
          <input
            ...
            placeholder="e.g. NTUC grocery run"
            required
            maxLength={120}
          />
        </label>
