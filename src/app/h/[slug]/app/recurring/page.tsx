...
export default async function Recurring({ params }: { params: { slug: string } }) {
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 pb-24 pt-6 sm:px-6">
      ...
      <Suspense fallback={<CardListSkeleton className="mt-4" rows={3} />}>
        <TemplateList slug={params.slug} houseId={house.id} houseMembers={houseMembers} />
      </Suspense>
    </main>
  );
}
...
async function TemplateList({
  ...
  const templates = await db().query.recurringTemplates.findMany({
    where: eq(recurringTemplates.houseId, houseId),
      ...
      {templates.map((t) => (
        <Card as="li" key={t.id}>
          <div className="flex items-center justify-between gap-3">
            <Link
              href={`/h/${slug}/app/recurring/${t.id}`}
